...
class Config(object):
    ...
    embedding_size = 50
    hidden_size = 200
    extension_size = 600 # 自己额外添加的一个隐层
    labmda = 10e-6 
    batch_size = 1024
    n_epochs = 10
    lr = 0.0005
...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.b2 = t.zeros((self.config.hidden_size, ), requires_grad=True)
        self.b3 = t.zeros((self.config.n_classes, ), requires_grad=True)
        self.W = xavier((self.config.n_features * self.config.embedding_size, self.config.extension_size))
        self.U = xavier((self.config.extension_size, self.config.hidden_size))
        self.V = xavier((self.config.hidden_size + self.config.extension_size, self.config.n_classes))
    ...
    def add_training_op(self):
        ...
        train_op = opt.Adam([self.W, self.b1, self.U, self.b2, self.V, self.b3,
    ...
    def fit(self, parser, save, train_examples, dev_set):
        ...
        for epoch in range(self.config.n_epochs):
            ...
            if dev_UAS > best_dev_UAS:
                ...
                if save:
                    t.save(self.W, './data/weights/W2.pth')
                    t.save(self.U, './data/weights/U2.pth')
                    t.save(self.embedded, './data/weights/embedded2.pth')
                    t.save(self.V, './data/weights/V2.pth')
                    t.save(self.b1, './data/weights/b1_ext.pth')
                    t.save(self.b2, './data/weights/b2_ext.pth')
                    t.save(self.b3, './data/weights/b3_ext.pth')
