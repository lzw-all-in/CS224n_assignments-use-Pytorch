...
def softmax(x):
    ...
    ### YOUR CODE HERE
    x_max = t.max(x, dim=1)[0][:, None]
    x_sub = x - x_max
    x_exp = t.exp(x_sub)
    sum_exp = t.sum(x_exp, dim=1)[:, None]
    out = x_exp / sum_exp
    ### END YOUR CODE
...
def test_softmax_basic():
    ...
    test1 = softmax(t.tensor(np.array([[1001, 1002], [3, 4]]), dtype=t.float32))
...
def test_cross_entropy_loss_basic():
    ...
    test1 = cross_entropy_loss(
            t.tensor(y, dtype=t.int32),
            t.tensor(yhat, dtype=t.float32))
