...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        diff_len = max_length - ori_len

        if diff_len > 0:
            padding_sentence = sentence + diff_len * [zero_vector]
            padding_lables  = labels +  diff_len * [zero_label]
            mask = ori_len * [True] + diff_len * [False]
        else:
            padding_sentence = sentence[0: max_length]
            padding_lables = labels[0: max_length]
            mask = max_length * [True]

        ret.append((padding_sentence, padding_lables, mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_training_op(self):
        ...
        train_op = opt.Adam([{'params': self.cell.parameters()}, {'params': self.U},
         {'params' : self.b2}, {'params' : self.embedded.weight}],
            lr=self.config.lr)
        ### END YOUR CODE
        return train_op
    ...
    def predict_on_batch(self, inputs_batch, mask_batch):
        ...
        self.mask = t.from_numpy(mask_batch.astype(np.float32))
        with t.no_grad():
            predictions = self.add_prediction_op(is_train=False)

        # 这里数据是三维的,并且最后一维才代表类别,所以axis=2
        return predictions.numpy().argmax(axis=2)


    def train_on_batch(self, inputs_batch, labels_batch, mask_batch):
        self.inputs = t.from_numpy(inputs_batch)
        self.labels = t.from_numpy(labels_batch)
        # 将True, False转化为0， 1的表达
        self.mask = t.from_numpy(mask_batch.astype(np.float32))
